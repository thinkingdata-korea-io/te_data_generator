...
**🆕 transactionStates** (선택사항, 고급 기능):
- 트랜잭션이 **여러 상태**를 가지는 경우 정의
- 각 상태에서 허용되는 이벤트를 명시적으로 제어
- **언제 사용?**
  - 복잡한 상태 전이가 있는 트랜잭션 (예: 게임 일시정지, 결제 취소)
  - 상태별로 허용 이벤트가 다른 경우
- **형식**:
  \`\`\`json
  "transactionStates": {
    "states": ["active", "paused", "resumed", "ended"],
    "allowedEvents": {
      "active": ["kill", "death", "pause"],
      "paused": ["resume", "quit"],
      "resumed": ["kill", "death", "pause"],
      "ended": []
    },
    "stateTransitions": {
      "active": ["paused", "ended"],
      "paused": ["resumed", "ended"],
      "resumed": ["paused", "ended"]
    }
  }
  \`\`\`
- **주의**: 대부분 트랜잭션은 단순 시작-종료로 충분, 필요한 경우만 사용
    ...
    "transactions": [
      // ⚠️ 필수 필드! 트랜잭션이 없으면 빈 배열 [] 로 응답
      {
        ...
        "transactionStates": {
          ...
          "allowedEvents": {
            "active": ["inner1", "inner2", "pause_event"],
            "paused": ["resume_event", "cancel_event"],
            "ended": []
          },
          "stateTransitions": {
            "active": ["paused", "ended"],
