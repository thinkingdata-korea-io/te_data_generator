...
export class AnalysisExcelParser {
  ...
  async parseAnalysisExcel(filePath: string): Promise<Partial<AIAnalysisResult>> {
    ...
    const segmentSheetName = '1_사용자_세그먼트';
    ...
    const sequencingSheetName = '2_이벤트_순서_규칙';
    ...
    const transactionSheetName = '3_트랜잭션_정의';
  ...
  private parseUserSegments(sheet: XLSX.WorkSheet): UserSegment[] {
    ...
    // Find the header row (contains "세그먼트명")
    let headerRowIndex = -1;
    for (let i = 0; i < data.length; i++) {
      if (data[i]?.includes('세그먼트명')) {
        headerRowIndex = i;
        break;
      }
    }
    ...
    for (let i = headerRowIndex + 1; i < data.length; i++) {
      ...
      if (!row || row.length === 0 || row[0]?.toString().startsWith('💡')) {
        break;
      }
  ...
  private parseEventSequencing(sheet: XLSX.WorkSheet): any {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      // Detect sections
      if (firstCell.includes('이벤트 카테고리')) {
        inCategorySection = true;
        inDependencySection = false;
        inConstraintSection = false;
        i++; // Skip header row
        continue;
      } else if (firstCell.includes('필수 선행 이벤트')) {
        inCategorySection = false;
        inDependencySection = true;
        inConstraintSection = false;
        i++; // Skip header row
        continue;
      } else if (firstCell.includes('실행 제약')) {
        inCategorySection = false;
        inDependencySection = false;
  ...
  private parseTransactions(sheet: XLSX.WorkSheet): Transaction[] {
    ...
    for (let i = headerRowIndex + 1; i < data.length; i++) {
      ...
      const allowInnerAfterEnd = row[5]?.toString().includes('허용');
